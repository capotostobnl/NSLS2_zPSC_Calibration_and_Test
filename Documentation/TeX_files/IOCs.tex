...
\section{Installing the ATE IOC}
	\begin{enumerate}
		...
		\item Clone the repo:\\
		\hspace*{1em} \texttt{git clone https://github.com/capotostobnl/ALSu-PSC\_ATE-ioc.git PSCtest}
		...
		\item \texttt{echo "EPICS\_BASE=/epics/base
		...
		\item Set the \texttt{NAME=PSCTest}, \texttt{PORT=} your next port
...
\begin{enumerate}
	...
	\item Create the \texttt{RELEASE.local} file, \texttt{touch configure/RELEASE.local}
	\item \texttt{echo "EPICS\_BASE = /epics/base}\\
		\hspace*{2em}\texttt{PSCDRV = /epics/modules/pscdrv}\\
	...
	\begin{enumerate}
		\item Edit the st.cmd file: \texttt{nano st.cmd}
		\item Update as needed
	\end{enumerate}
	
	\item Find the next available port: \texttt{manage-iocs nextport}
	\item Configure this in the config file: \texttt{nano config}
	\item Set the \texttt{NAME=zpsc\_ioc}, \texttt{PORT=} your next port
