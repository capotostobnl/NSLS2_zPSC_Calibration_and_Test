...
\begin{enumerate}
	...
	\item Create the directory: \texttt{sudo mkdir /opt/phoebus}\newline
		\hspace*{2em}\texttt{sudo chown \$USER:epics /opt/phoebus}\newline
...
\begin{itemize}
	\item[] \texttt{cd /opt/phoebus}
	\item[]\texttt{wget }\path|https://github.com/ControlSystemStudio/phoebus/releases/download/v4.7.4/phoebus-4.7.4-linux.tar.gz|
	\item[] \texttt{tar -xvzf phoebus-4.7.4-linux.tar.gz -C . --strip-components=1}
\end{itemize}
	...
	\item Copy in necessary configuration changes from the template:\\
		...
		\hspace*{2em}\texttt{https://github.com/capotosto/Phoebus-ALSU.git /opt/phoebus/config}
	\item overwrite the defaults with the new template: \texttt{mv config/* .}
	\item Update paths as necessary in the \texttt{settings.ini}
	\item Alias Phoebus to run via command \texttt{run-phoebus}:\\
		\hspace*{2em}\texttt{echo 'alias run-phoebus="/opt/phoebus/phoebus.sh"' >> \textasciitilde/.bashrc}\\
