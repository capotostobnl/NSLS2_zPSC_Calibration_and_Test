...
\begin{center}
	...
	\fbox{
		\parbox{0.9\textwidth}{
			\textbf{It is assumed that secondary ops have been completed for the PSC to be tested. This will include installing fuses, setting various jumpers, verifying various voltages, etc. Refer to the example secondary ops checklist in \appref{app:Example Secondary Ops Checklist}\\\\The EEPROM must be configured in the PSC prior to using any of these scripts. Failure to configure the EEPROM, or incorrectly configuring it, will cause issues with the script at even the first menu selection stages. Refer to \autoref{chap:Initial Booting of the PSC and Configuration of the EEPROM} for instructions}
		}
	}
\end{center}
